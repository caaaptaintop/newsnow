...
import { claimCollection, finishCollection, pollCollection } from "../../source-admin/collection-jobs"
...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const operations: Record<string, string> = { "init": "migrate", "legacy": "migrate", "activate": "migrate", "publish": "publish", "known": "publish", "export": "publish", "version": "publish", "collection-poll": "operate", "collection-claim": "operate", "collection-finish": "operate", "status": "operate", "relay": "operate", "maintain": "operate", "source-tests": "runtime-source-test", "source-test-result": "runtime-source-test" }
    ...
    if (body.action.startsWith("collection-") && owner === "deployment") throw new BuildingError(403, "采集任务仅允许签名机器执行")
    const db = buildingDB(event)
    switch (body.action) {
      case "collection-poll":return pollCollection(event)
      case "collection-claim":return claimCollection(event, owner, body.id)
      ...
      case "legacy":{
        ...
        return { articles: rows.map((r) => {
          try {
            ...
            throw new BuildingError(409, "旧数据JSON损坏，迁移已停止")
          }
        }), after: rows.length === 80 ? rows[79].id : null }
      }
      ...
      case "maintain":await maintainBuilding(db)
        return { ok: true }
