...
import { applyIntelligenceSourceConfig, customSourceSeed, sourceConfigCanPublish, validateIntelligenceSourceConfig } from "../../shared/source-config"
...
export async function testSourceConfig(value: IntelligenceSourceConfig): Promise<SourceConfigTestResult> {
  const seed = intelligenceSources.find(source => source.id === value.id && source.topic === value.topic) ?? customSourceSeed(value)
  ...
  if (config.collectionMode === "discover") {
    ...
    return { schemaVersion: 1, ok: candidates.length > 0, publishable: false, mode: "discover", endpoints: [], candidates, message: candidates.length ? "仅发现候选。请选择实际栏目，切换到明确栏目后重新测试" : "未发现可用栏目" }
  }
  ...
  for (const endpoint of config.endpoints.filter(item => item.enabled)) {
    ...
    try {
      if (Date.now() >= deadline) {
        ...
        throw new Error("本次测试达到时间预算；此栏目未测试")
      }
