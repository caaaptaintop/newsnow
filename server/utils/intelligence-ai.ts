import { type IntelligenceTopic, intelligenceContentTypes, intelligenceTopics } from "@shared/intelligence"
...
export function intelligenceParseAI(result: any): unknown[] {
  ...
  if (typeof raw === "string") {
    ...
    try {
      ...
      throw new Error("AI 没有返回有效 JSON，本批未入库")
    }
  }
...
export function intelligenceNormalizeDecision(raw: any, validKeys: Set<string>, topic: IntelligenceTopic): IntelligenceDecision | undefined {
  ...
  if (!categories.includes(raw.category) || !(intelligenceContentTypes as readonly string[]).includes(raw.contentType) || (topic === "building" && raw.contentType === "热点选题")) return
  ...
  const relatedCategories = topic === "building" ? [] : strings(raw.relatedCategories, 3).filter(c => categories.includes(c) && c !== raw.category)
  const tags = topic === "building" ? [] : strings(raw.tags, 6)
  return { ...base, category: raw.category, relatedCategories, tags, contentType: raw.contentType, importance: Math.max(0, Math.min(100, Math.round(importance))), summary }
}
export async function intelligenceClassify(ai: any, topic: IntelligenceTopic, items: { key: string, title: string, body?: string, column: string }[]) {
  const building = topic === "building"
  const relatedRule = building
    ? "建筑每篇文章只属于一个主栏目；relatedCategories 必须为空数组；不要生成标签，tags 必须为空数组。"
    : "一个主栏目，最多三个实质相关栏目；标签不替代栏目。"
  const keepExample = building
