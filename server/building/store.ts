...
import { type BatchItem, BuildingError, buildingHash, buildingLimits, canonicalItems, normalizeBatchItem, publicArticle } from "../../shared/building-contract"
import { intelligenceSources } from "../../shared/official-sources"
import { type PublishedSourceConfig, customSourceSeed, intelligenceSourceCollectionScope, intelligenceSourceConfigHash, sourceConfigHost } from "../../shared/source-config"
...
export function buildingEnv(event: any): Record<string, any> {
  return event.context.cloudflare?.env ?? event.context.env ?? {}
}
...
export async function publishBatch(db: BuildingDB, owner: string, body: any, configs: PublishedSourceConfig[] = []) {
  ...
  const custom = configs.filter(config => config.id.startsWith("custom-") && config.topic === "building" && config.enabled && config.collectionApproved === true)
  const allowed = new Set([...intelligenceSources.filter(isPublishedSource).map(s => s.id), ...custom.map(s => s.id)])
  for (const i of items) {
    if ((i.kind === "article" && !allowed.has(i.data.sourceId)) || (i.kind === "source" && !allowed.has(i.key))) throw new BuildingError(400, "来源未启用")
  }
  for (const item of items.filter(item => item.kind === "article" && item.data.sourceId.startsWith("custom-"))) {
    const config = custom.find(config => config.id === item.data.sourceId)!
    if (sourceConfigHost(item.data.url) !== sourceConfigHost(config.home) || !config.endpoints.some(endpoint => endpoint.enabled && endpoint.name === item.data.column)) throw new BuildingError(400, "文章不属于当前确认的来源栏目")
  }
  const usedCustom = custom.filter(c => items.some(i => (i.kind === "article" && i.data.sourceId === c.id) || (i.kind === "source" && i.key === c.id)))
  const scopes: Record<string, string> = {}
  for (const config of usedCustom) {
    const scope = await buildingHash(intelligenceSourceCollectionScope(customSourceSeed(config)!))
    if (body.sourceScopes?.[config.id] !== scope) throw new BuildingError(409, "采集来源配置已变化，请重新采集后发布")
    scopes[config.id] = scope
  }
  const hash = await buildingHash(JSON.stringify([body.baseRevision, items, ...(usedCustom.length ? [scopes] : [])]))
  ...
  if (prior) {
    if (prior.owner !== owner || prior.hash !== hash) throw new BuildingError(409, "批次编号已用于其他内容")
    return { revision: prior.revision, repeated: true }
  }
  ...
  const revision = state.revision + (visibleChange || !state.initialized ? 1 : 0)
  ...
  const customGuards: string[] = []
  for (const config of usedCustom) {
    const id = `${guard}:${config.id}`
    customGuards.push(id)
    const { collectionApproved: _approval, ...normalized } = config
    const configHash = await intelligenceSourceConfigHash(normalized)
    statements.push(db.prepare(`INSERT INTO building_guards_v3 VALUES (?, CASE WHEN EXISTS (
      SELECT 1 FROM intelligence_source_config_entry e JOIN intelligence_source_config_revision r
      ON r.topic=e.topic AND r.source_id=e.source_id AND r.revision=e.active_revision
      WHERE e.topic='building' AND e.source_id=? AND r.config_hash=? AND r.reason='collection-confirmed'
    ) THEN 1 ELSE 0 END)`).bind(id, config.id, configHash))
  }
  for (const i of items) {
    const d = i.data
    if (i.kind === "article") {
    ...
    } else if (i.kind === "decision") {
      statements.push(db.prepare(`INSERT INTO building_seen_v3 VALUES(?,?,?) ON CONFLICT(id) DO UPDATE SET title=excluded.title,seen_at=excluded.seen_at WHERE excluded.seen_at>=building_seen_v3.seen_at`).bind(i.key, d.title, d.at))
    } else {
  ...
  for (const id of customGuards) statements.push(db.prepare("DELETE FROM building_guards_v3 WHERE id=?").bind(id))
  try {
...
export async function knownRecords(db: BuildingDB, keys: any) {
  ...
  for (const r of [...result[0].results, ...result[1].results]) {
    if (!rows.has(r.key) || rows.get(r.key).at < r.at)rows.set(r.key, r)
  }
...
export async function activateBuilding(db: BuildingDB, expectedTotal: number, revision: number) {
  ...
  try {
    ...
    if (/CHECK constraint failed/i.test(String(error))) throw new BuildingError(409, "迁移验收数量或版本不一致")
    throw error
  }
