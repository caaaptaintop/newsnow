...
export async function readPage(db: BuildingDB, params: URLSearchParams, now = Date.now()) {
  ...
  const hash = await buildingHash(JSON.stringify([q.filters, q.limit]))
  ...
  const { where, binds } = conditions(q.filters, at)
  ...
  const result = await db.batch([
    ...
    db.prepare("SELECT category,COUNT(*) AS n FROM building_docs_v3 GROUP BY category"),
...
export async function readVersion(db: BuildingDB) {
