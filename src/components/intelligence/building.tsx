...
import { buildingView } from "@shared/building-view"
import { type IntelligenceFilters, emptyIntelligenceFilters, intelligenceContentTypes, intelligenceHttpUrl, intelligenceTopics } from "@shared/intelligence"
...
function Brand() {
  return (
    <header className="intel-topbar">
      <a href="/?topic=building" className="intel-brand">
        <span className="intel-brand-mark">情</span>
        <span>
          个人信息情报站
          <small>CAPX · INTELLIGENCE</small>
        </span>
      </a>
      <span className="intel-current-topic">建筑情报</span>
    </header>
  )
}
    ...
    <article className="intel-card">
      ...
      {!!article.otherSources?.length && (
        <details className="intel-attachments">
          ...
          {article.otherSources.map(source => <a key={source.url} href={source.url} target="_blank" rel="noreferrer">{source.name}</a>)}
        </details>
      )}
    </article>
  )
...
export function BuildingWorkspace() {
  ...
  useEffect(() => {
    const timer = setTimeout(() => setSearch(filters.q.trim()), 300)
  ...
  }, [filters.category, filters.sort, filters.days, filters.importance, filters.regions, filters.cities, filters.types, filters.sources, search])
  const feed = useInfiniteQuery({
    queryKey: ["public-intelligence-v3", topic, queryParams, refreshKey],
    initialPageParam: "",
    ...
    enabled: ready && !unavailable && search === filters.q.trim(),
    staleTime: Infinity,
    gcTime: 60000,
  ...
  const patch = (change: Partial<IntelligenceFilters>) => {
    setView(current => ({ ...current, filters: { ...current.filters, ...change } }))
  }
  const refresh = async () => {
  ...
  useEffect(() => {
    setView(buildingView(window.location.search))
    setReady(true)
  }, [])
  useEffect(() => {
    if (!ready) return
  ...
  useEffect(() => {
    ...
    const outside = (event: Event) => {
      if (!(event.target instanceof Element) || !event.target.closest(".intel-filter")) setActiveFilter(undefined)
    }
  ...
  const selected = (["regions", "cities", "types", "sources"] as const).flatMap(key => filters[key].map(value => ({ key, value, label: key === "sources"
    ? sourceOptions.find(option => option.id === value)?.name ?? value
    : key === "cities"
      ? (() => {
          try {
            const pair = JSON.parse(value)
            return Array.isArray(pair) ? pair.join(" / ") : value
          } catch {
            return value
          }
        })()
      ...
      <div className="intel-app">
        ...
        <section className="intel-main">
          <div className="intel-empty">
            <h1>该主题暂未开放</h1>
            <p>本站目前仅提供建筑资讯。其他主题已暂停，后续按需恢复。</p>
            <a className="intel-button" href="/?topic=building">返回建筑情报</a>
          </div>
        </section>
      </div>
    )
        ...
        <aside className="intel-sidebar">
          ...
          <nav aria-label="建筑栏目">
            <button className={!filters.category ? "is-active" : ""} type="button" onClick={() => patch({ category: "" })}>
              <span>全部信息</span>
              <small>{totalPublished}</small>
            </button>
            {Object.entries(categories).map(([id, name]) => (
              <button type="button" key={id} className={filters.category === id ? "is-active" : ""} onClick={() => patch({ category: id })}>
                <span>{name}</span>
        ...
        <section className="intel-main">
          ...
          <div className="intel-controls">
            <form className="intel-search" onSubmit={event => event.preventDefault()} role="search">
              <Icon kind="search" />
              <input type="search" aria-label="搜索当前栏目及筛选条件内的信息" placeholder="在当前栏目与筛选条件内搜索标题、摘要、文号…" maxLength={200} value={filters.q} onChange={event => patch({ q: event.target.value })} />
              <kbd>当前范围</kbd>
            </form>
            <div className="intel-filter-row">
              ...
              <label className="intel-select">
                <span>重要度</span>
                <select aria-label="重要度" value={filters.importance} onChange={event => patch({ importance: Number(event.target.value) })}>
                  <option value="0">全部</option>
                  <option value="60">值得关注及以上</option>
                  <option value="80">重点关注</option>
                </select>
              </label>
            </div>
            {(selected.length > 0 || filters.days > 0 || filters.importance > 0 || filters.q) && (
              <div className="intel-selected">
                <span>已选</span>
                {selected.map(item => (
                  <button type="button" key={`${item.key}:${item.value}`} onClick={() => patch({ [item.key]: filters[item.key].filter(value => value !== item.value) })}>
                    {item.label}
                    <span aria-label="移除">×</span>
                  </button>
                ))}
                {filters.days > 0 && (
                  <button type="button" onClick={() => patch({ days: 0 })}>
                    最近
                    {filters.days}
                    {" "}
                    天 ×
                ...
                {filters.importance > 0 && (
                  <button type="button" onClick={() => patch({ importance: 0 })}>
                    {filters.importance === 80 ? "重点关注" : "值得关注及以上"}
                    {" "}
                    ×
                  </button>
                )}
                {filters.q && (
                  <button type="button" onClick={() => patch({ q: "" })}>
                    搜索：
                    {filters.q}
                    {" "}
                    ×
                ...
                <button type="button" className="intel-text-button" onClick={reset}>清除筛选</button>
              </div>
            )}
          </div>
          <div className="intel-results-bar">
            <p>
              <strong>{total}</strong>
              {" "}
              条信息
            ...
            <label>
              排序
              <select aria-label="结果排序" value={filters.sort} onChange={event => patch({ sort: event.target.value as IntelligenceFilters["sort"] })}>
          ...
          <div className="intel-sync-status" role="status">
            <span>
              最近成功发布：
              {updatedAt ? new Date(updatedAt).toLocaleString("zh-CN", { timeZone: "Asia/Shanghai" }) : "待确认"}
            </span>
            {version.isError && <span>暂时无法检查更新，保留当前内容。</span>}
            {newer && <button type="button" className="intel-text-button" disabled={feed.isFetching} onClick={() => void refresh()}>有新资讯，点击更新</button>}
          </div>
          {feed.isError && (
            <div className="intel-warning" role="alert">
              {feed.error.message}
              {articles.length > 0 && "，当前保留上次读取的内容。"}
            </div>
          )}
          ...
          {ready && !feed.isFetching && !feed.isError && !total && (
            <div className="intel-empty">
              <h2>{totalPublished ? "当前条件下没有结果" : "暂无已发布资讯"}</h2>
              <p>{totalPublished ? "可减少筛选条件或清除搜索词；发布日期未核实的内容不出现在限定日期的结果中。" : "后台尚未发布可展示的建筑资讯，请稍后刷新。"}</p>
              {totalPublished > 0 && <button type="button" className="intel-button" onClick={reset}>清除筛选</button>}
            </div>
          )}
