...
export default function AttachmentReader({ target, onClose }: { target: AttachmentPreviewTarget, onClose: () => void }) {
  ...
  return createPortal(<dialog ref={dialog} className="intel-preview-dialog" aria-labelledby={headingId}>
    <header className="intel-preview-header"><div><h2 id={headingId}>{preview?.filename ?? target.title}</h2><p>{target.sourceName} · 按需读取，不保存附件</p></div><form method="dialog"><button type="submit" aria-label="关闭附件预览"><svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" strokeWidth="1.8" aria-hidden="true"><path d="m6 6 12 12M18 6 6 18" /></svg></button></form></header>
    ...
    <div className="intel-preview-content" aria-busy={!!stage}>
      {stage && <div className="intel-preview-message" role="status"><p>{stage}</p><form method="dialog"><button type="submit">取消预览</button></form></div>}
