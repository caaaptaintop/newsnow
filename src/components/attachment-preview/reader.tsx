...
export default function AttachmentReader({ target, onClose }: { target: AttachmentPreviewTarget, onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (detected.format === "pdf") throw new Error("此附件实际为 PDF，请点击原站下载在新标签页打开官方文件")
        let html = ""
  ...
  return createPortal(<dialog ref={dialog} className="intel-preview-dialog" aria-labelledby={headingId}>
    <header className="intel-preview-header"><div><h2 id={headingId}>{preview?.filename ?? target.title}</h2><p>{target.sourceName} · 按需读取 · 临时缓存最多 5 分钟</p></div><button type="button" data-preview-close-button aria-label="关闭附件预览"><svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" strokeWidth="1.8" aria-hidden="true"><path d="m6 6 12 12M18 6 6 18" /></svg></button></header>
    ...
    <div className="intel-preview-content" aria-busy={!!stage}>
      ...
      {error && <div className="intel-preview-message" role="alert"><h3>无法在线预览</h3><p>{error}</p><p>可直接打开原网页查看，或下载原文件；无需排队。</p>{originPdf && browserCanPreviewPdf && <><a data-preview-origin-pdf className="intel-preview-origin-link" href={originPdf} target="_blank" rel="noreferrer">在新标签页预览原站 PDF</a><p>此方式由浏览器直接读取政府网站 PDF，不经过本站附件转发；部分政府网站禁止跨站嵌入，因此使用新标签页。</p></>}<button type="button" onClick={() => setAttempt(value => value + 1)}>重试本站预览</button></div>}
    ...
    <footer className="intel-preview-footer">{preview?.via === "cache" ? "读取方式：短期缓存 → 浏览器" : preview?.via === "relay" ? "读取方式：原站转发 → 浏览器" : preview?.via === "direct" ? "读取方式：原站 → 浏览器" : "仅在点击预览后读取附件"}。{preview && ["doc", "docx", "word-html"].includes(preview.format) ? "阅读级预览；复杂图形、修订和分页可能与原件不同，以原文件为准。" : "关闭后释放本次预览资源。"}</footer>
