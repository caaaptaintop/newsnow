...
def get_json(url, timeout=30):
    req = urllib.request.Request(url, headers={"User-Agent": "newsnow-jianing-ab/2.0"})
...
def post_json(url, body, timeout=180):
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "newsnow-jianing-ab/2.0",
        },
    )
    started = time.time()
    try:
...
print("===== JIANING MODEL A/B SUMMARY =====")
print(f"Live candidate count: {len(payload_items)} (top {PER_SOURCE} per source before cross-source dedupe)")
