...
import { buildingStatus, maintainBuilding, relayPolicy, reserveRelay, settleRelay } from "../server/building/relay-budget"
import { buildingLimits, locationCityId, normalizeBatchItem } from "../shared/building-contract"
...
async function publish(items: any[]) {
  ...
  for (let i = 0; i < items.length; i += buildingLimits.batchItems) {
    result = await publishBatch(memory.db, "test", { batchId: `testbatch_${++counter}`, baseRevision: (await buildingMeta(memory.db)).revision, items: items.slice(i, i + buildingLimits.batchItems) })
  }
  const state = await buildingMeta(memory.db)
  if (!state.migrationComplete && state.total > 0) await activateBuilding(memory.db, state.total, state.revision)
  return result
}
describe("atomic metadata publication", () => {
  ...
  it("state-only, rejected decisions and identical articles do not change public version", async () => {
    ...
    const v = await readVersion(memory.db)
  ...
  it("late older records cannot replace newer corrections", async () => {
    await publish([article(1, { collectedAt: now + 100, title: "新修正" })])
    await publish([article(1, { title: "旧数据" })])
  ...
  it("strips original text and file bytes; public projection hides model details", async () => {
    ...
    expect(JSON.stringify(a)).not.toContain("neverpersist")
    await publish([a])
...
describe("sql pagination and full-range filters", () => {
  it("returns true pages with no duplicates and full totals", async () => {
    ...
    const first = await readPage(memory.db, new URLSearchParams(), now)
    expect(first.articles).toHaveLength(50)
    expect(first.total).toBe(115)
    const second = await readPage(memory.db, new URLSearchParams({ cursor: first.nextCursor! }), now)
    expect(second.articles).toHaveLength(50)
    const third = await readPage(memory.db, new URLSearchParams({ cursor: second.nextCursor! }), now)
    expect(third.articles).toHaveLength(15)
  ...
  it("matches province OR qualified city without same-name city collisions", async () => {
    ...
    expect(page.facets.locations.some(g => g.region === "其他")).toBe(true)
  })
  ...
  it("supports primary category, group filters and ranked cursors", async () => {
    await publish([article(1, { importance: 90 }), article(2, { importance: 90 }), article(3, { importance: 60 })])
    const params = new URLSearchParams({ category: "intelligent_construction", sources: "住建官方", importance: "80", sort: "importance", limit: "1" })
    const first = await readPage(memory.db, params, now)
    expect(first.total).toBe(2)
    params.set("cursor", first.nextCursor!)
    const second = await readPage(memory.db, params, now)
  ...
  it("ignores leftover tags query parameters", async () => {
    await publish([article(1, { tags: ["BIM"] }), article(2, { tags: ["装配式"] })])
    const withTags = await readPage(memory.db, new URLSearchParams({ tags: "BIM" }), now)
    const without = await readPage(memory.db, new URLSearchParams(), now)
    expect(withTags.total).toBe(2)
    expect(withTags.articles.map(a => a.key)).toEqual(without.articles.map(a => a.key))
  })
  it.each([{ limit: "101" }, { sort: "bad" }, { days: "3" }, { category: "constructor" }, { cursor: "broken" }])("rejects invalid queries %j", async (query) => {
    await publish([article(1)])
    await expect(readPage(memory.db, new URLSearchParams(Object.entries(query).filter((entry): entry is [string, string] => typeof entry[1] === "string")), now)).rejects.toMatchObject({ statusCode: 400 })
  })
})
describe("durable relay limits", () => {
  ...
  it("enforces concurrent reservations with idempotent settlement", async () => {
    ...
    const one = await reserveRelay(memory.db, "ip", "salt", size, now)
    const two = await reserveRelay(memory.db, "ip", "salt", size, now)
    await expect(reserveRelay(memory.db, "ip", "salt", size, now)).rejects.toMatchObject({ statusCode: 429 })
    await settleRelay(memory.db, one, 1024, false)
    await settleRelay(memory.db, one, 1024, false)
    const stats = await buildingStatus(memory.db)
    expect(stats.usage[0].reserved).toBe(size + 1024)
    expect(stats.usage[0].delivered).toBe(1024)
    await settleRelay(memory.db, two, 0, true)
  })
  it("does not refund unknown consumption when a lease expires", async () => {
    const size = 20 * 1024 * 1024
    await relayPolicy(memory.db, { enabled: true, bytesPerDay: size })
  ...
  it("pause stops new relays without stopping public reads", async () => {
    await publish([article(1)])
    await relayPolicy(memory.db, { enabled: false })
    ...
    await relayPolicy(memory.db, { enabled: true })
    expect(await reserveRelay(memory.db, "ip", "salt", 1024, now)).toBeTruthy()
  })
  it("bounded maintenance preserves public metadata", async () => {
    await publish([article(1)])
    await maintainBuilding(memory.db)
    expect((await readPage(memory.db, new URLSearchParams(), now)).articles).toHaveLength(1)
  })
})
