...
export async function validateSourceEnvelope(value: unknown): Promise<PublishedSourceConfigEnvelope> {
  ...
  const sources = input.sources.map((value) => {
    ...
    if (value.collectionApproved !== undefined && typeof value.collectionApproved !== "boolean") throw new Error("Invalid collection approval")
    return { ...config, ...(value.collectionApproved !== undefined ? { collectionApproved: value.collectionApproved } : {}) }
  })
...
async function responseEnvelope(response: Response) {
  ...
  try {
    while (true) {
      ...
      if (size > sourceConfigPolicy.maxResponseBytes) {
        await reader.cancel()
        throw new Error("Source configuration response too large")
      }
...
async function refresh(cachedOnly: boolean): Promise<Catalog> {
  const make = (envelope: PublishedSourceConfigEnvelope | undefined, origin: Catalog["origin"]): Catalog => ({
    configs: new Map((envelope?.sources ?? []).map(config => [config.id, config])),
  ...
  try {
    ...
    try {
      unlinkSync(pending)
    } catch { /* Only this invocation's temporary file is eligible for cleanup. */ }
...
/** Production collection fails closed for seeds and old catalogs without approval. */
export async function resolveCollectionSource(source: IntelligenceSource, cachedOnly = false) {
  const approved = (await catalog(cachedOnly)).configs.get(source.id) as (IntelligenceSourceConfig & { collectionApproved?: boolean }) | undefined
  const configured = approved ? applyIntelligenceSourceConfig(source, approved) : source
  return { ...configured, enabled: source.topic === "building" && approved?.collectionApproved === true && configured.enabled }
}
