...
const batch = JSON.parse(await readFile(resolve(dir, "result.json"), "utf8"))
...
for (const a of result.articles) {
  if (a.topic === "building" && sources.has(a.sourceId)) {
    const item = normalizeBatchItem({ kind: "article", key: a.key, data: a })
    if (old.get(a.key) !== JSON.stringify(item))items.push(item)
  }
}
for (const d of batch.decisions ?? []) {
  if (sources.has(d.sourceId) && /^building:[a-f0-9]{64}$/.test(d.key) && typeof d.title === "string")items.push(normalizeBatchItem({ kind: "decision", key: d.key, data: { title: d.title, at: d.at } }))
}
for (const s of batch.states ?? []) {
  if (sources.has(s.id))items.push(normalizeBatchItem({ kind: "source", key: s.id, data: s }))
}
...
for (let i = 0;
  ...
  await atomicJson(outboxPath, null)
  published += chunk.length
  for (const item of chunk) {
    if (item.kind === "article") {
      if (old.has(item.key))updatedArticles++
      else publishedArticles++
    }
  }
}
...
console.log(JSON.stringify({ published, publishedArticles, updatedArticles, storage: "D1", codeDeployment: false }))
