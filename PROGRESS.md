...
项目开始迁移为与 AI Cube 同款的 local-first 开发治理：产品架构与既有安全边界不变，GitHub Issue / Draft PR / main 正式治理不变；连续开发、测试和中间修改默认在明确本地 worktree 与独立 branch 完成，达到 clean fixed candidate 后再同步 GitHub。GitHub Actions 不再作为每个中间改动的必要前置，而用于 fixed candidate 的共享验证。
...
- 正式链路为 `Issue → branch/worktree → local develop/test → fixed candidate → Draft PR → independent review → final Head → main`；
...
Issue #74 / PR #76 在流程迁移启动前已经进入住建部动态列表/分页实网验收。该产品线继续沿用既有线程、任务和验收口径完成，不因 #77 中途改变现场。#77 合并后，新的开发任务默认使用 local-first 流程。
