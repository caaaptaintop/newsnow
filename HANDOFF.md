...
- 实际任务指令、原始证据和审查结论：当前功能 PR 或下述备用交接 PR

## 当前备用交接 PR

当某个本机任务没有对应的功能 PR 时，统一使用：

- PR：`#70` — `docs: use GitHub for ChatGPT-Codex handoff`
- URL：`https://github.com/caaaptaintop/newsnow/pull/70`
- 状态说明：该 PR 已合并，但其会话继续作为备用交接总线使用；不要锁定会话。若未来不可评论，由 ChatGPT 建立新的备用交接 PR 并更新本文件。

本仓库当前关闭 GitHub Issues，因此不要把 Issue 作为默认交接前提。
...
1. 有直接相关的功能 PR：ChatGPT 使用该 PR。
2. 没有相关功能 PR：ChatGPT 使用上面的备用交接 PR。
3. ChatGPT 在对应 PR 发布 `[CHATGPT→CODEX][TASK <id>]` 完整指令。
4. Codex 从 GitHub 读取该指令，仅执行明确允许的本机操作。
5. Codex 在同一 PR 发布 `[CODEX→CHATGPT][TASK <id>]`，附可复核的原始事实、日志摘录、SHA/哈希和 unknown。
6. ChatGPT 独立读取源码、GitHub 证据和必要线上状态，发布 `[CHATGPT REVIEW][TASK <id>]` 并继续修改、PR、合并、部署或下一轮本机验证。

用户通常只需要把 PR 编号和 Task ID 交给本机 Codex，不需要搬运文件。
...
再读取 ChatGPT 指定的功能 PR，或本文件中的备用交接 PR：
...
如果 `gh auth status` 失败、仓库不可访问、PR 不可评论、基线 SHA 不一致、现场与指令冲突或执行将触碰禁止边界，应停止并报告，不凭历史聊天猜测。

## 3. Codex 回传格式

优先生成一个临时、已脱敏的 Markdown 结果，再直接发到同一 PR：
...
- 小型文本、状态、命令输出、日志摘录 → 当前功能 PR 或备用交接 PR 评论；
...
只有 GitHub PR 会话、Actions、PR diff、哈希和必要摘录仍不足以完成审查时，才例外由 ChatGPT 明确要求文件本体。

## 5. 职责边界

Codex 的 GitHub 写入默认仅限：

- 在被指定的功能 PR 或备用交接 PR 回传本机结果；
