...
const feed = { topic: "building", version: "v1", updatedAt: 1788960000000, articles, sources, truncated: false, totalPublished: articles.length, nextCursor: null, facets: { categories: { intelligent_construction: 3 }, locations: [{ region: "江苏", cities: ["南京", "苏州"] }, { region: "浙江", cities: ["杭州"] }], tags: [{ id: "BIM", name: "BIM" }] } }
function filteredFeed(params) {
  const regions = params.getAll("regions")
  const cities = params.getAll("cities")
  const rows = articles.filter(a => (!regions.length && !cities.length) || regions.includes(a.region) || cities.includes(JSON.stringify([a.region, a.city])) || cities.includes(a.city))
  return { ...feed, articles: rows, total: rows.length }
}
const server = createServer(async (req, res) => {
  try {
    ...
    if (url.pathname.startsWith("/api/")) {
      ...
      const data = url.pathname === "/api/intelligence" ? filteredFeed(url.searchParams) : url.pathname === "/api/intelligence/version" ? { topic: "building", version: feed.version, updatedAt: feed.updatedAt } : { error: "Unexpected API" }
      res.writeHead(url.pathname === "/api/intelligence" || url.pathname === "/api/intelligence/version" ? 200 : 404, { "Content-Type": "application/json", "Cache-Control": "no-store" }).end(JSON.stringify(data))
      return
    }
...
try {
  ...
  assert.equal(await evaluate("!!document.querySelector('.intel-quick-tags')"), false, "no quick tags")
  ...
  assert.equal(await evaluate("document.body.innerText.includes('BIM')"), false)
  await send("Page.navigate", { url: "http://127.0.0.1:4174/" })
  await until("document.querySelectorAll('.intel-card').length===3", "building feed after tag URL")
