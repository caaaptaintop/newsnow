...
const server = createServer(async (req, res) => {
  try {
    ...
    if (url.pathname === "/api/intelligence/attachment") {
      ...
      const key = new URL(body.url).searchParams.get("id")
      const file = fixtures[key]

      assert(file)

      res.writeHead(200, { "Content-Type": file.mime, "Content-Disposition": `inline; filename*=UTF-8''${encodeURIComponent(file.filename)}`, "Cache-Control": "no-store" })
      res.end(file.bytes)
      return
    }
...
ws.addEventListener("message", async (event) => {
  ...
  if (message.method === "Page.windowOpen" && message.params.url === fixtureUrl("pdf")) pdfWindows.push(message.params.url)
...
try {
  ...
  const pdfLink = await evaluate(`(() => {
    const link = [...document.querySelectorAll('.intel-preview-open')].find(b => b.textContent.includes(${JSON.stringify(fixtures.pdf.filename)}));
    return { href: link.href, target: link.target };
  })()`)

  assert.deepEqual(pdfLink, { href: pdfUrl, target: "_blank" })

  await send("Runtime.evaluate", { expression: `[...document.querySelectorAll('.intel-preview-open')].find(b => b.textContent.includes(${JSON.stringify(fixtures.pdf.filename)})).click()`, userGesture: true })

  for (let i = 0; i < 40 && !pdfWindows.length; i++) await sleep(50)

  assert.deepEqual(pdfWindows, [pdfUrl], "PDF click opens the official URL in a new window")

  assert.equal(await evaluate("!!document.querySelector('.intel-preview-dialog')"), false, "PDF bypasses the site dialog")

  assert.equal(relayRequests.length, beforePdf, "PDF bypasses the site relay")

  // Synthetic DNS is not a real official PDF rendering test.
