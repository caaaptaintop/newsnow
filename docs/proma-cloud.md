...
状态：PENDING，代码已支持；真实调用需生产密钥和部署验收。
...
| PROMA_MODEL | gpt-5.6-luna | 普通变量，可省略，代码默认此值 |
...
- 所有情报主题和健宁分类共用服务端模型配置，低推理强度，单次连接超时 25 秒。
...
2. `/api/topics/health/status` 显示 `provider=proma`、`model=gpt-5.6-luna`、`enabled=true` 只表示配置完整。
