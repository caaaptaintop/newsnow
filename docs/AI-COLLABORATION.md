...
ChatGPT 合并与云端部署检查
    ↓
如仍有本机独占事项 → 在对应 GitHub Issue/PR 发布 Codex 指令
    ↓
Codex 直接在该 GitHub 线程回传本机证据
    ↓
ChatGPT 复审并决定下一步
```
...
确认问题必须依赖用户 Mac
    ↓
ChatGPT 选择已有 Issue/PR 或新建 GitHub Issue
    ↓
在该线程发布 [CHATGPT→CODEX] 指令
    ↓
Codex 只复现、观察或执行明确的本机动作
    ↓
Codex 在同一线程发布 [CODEX→CHATGPT] 原始结果
    ↓
ChatGPT 独立复审并发布 [CHATGPT REVIEW]
    ↓
...
1. 当前已有 PR，且本机验证直接对应该 PR：使用该 PR；
2. 当前已有 Issue，且本机验证对应该问题：使用该 Issue；
3. 没有合适线程：由 ChatGPT 新建一个范围明确的 Issue。

不为同一轮任务重复建立多个交接线程。根目录 `HANDOFF.md` 是入口与协议索引，不是动态日志仓库。

### 2. 三种固定标记

- `[CHATGPT→CODEX]`：ChatGPT 发布本机执行指令；
- `[CODEX→CHATGPT]`：Codex 回传执行结果和原始证据；
- `[CHATGPT REVIEW]`：ChatGPT 独立审查、问题分级和下一步结论。

这样任何一方只看 GitHub 线程即可恢复上下文，不依赖聊天记录或用户手工转发附件。
...
然后读取 ChatGPT 指定 Issue/PR 的最新评论，例如：
...
如果 `gh` 未认证、仓库不可访问或最新指令与本机状态冲突，Codex 应停止，不得用旧聊天中的大段指令猜测执行。

### 4. Codex 回传方式

正常情况下，Codex 直接写回同一线程：
...
只有 GitHub 线程、Actions、PR diff、哈希和必要摘录仍不足以完成审查时，才例外要求用户手工上传文件本体。

## 七、Codex 指令应短而封闭

给 Codex 的任务应是明确的本机操作包，而不是“继续开发整个项目”。完整指令应发布到 GitHub 线程；聊天里通常只需要给用户一个 Issue/PR 链接或编号。
...
版本变化和新增/修改 ID，并直接回传到本 Issue；本机敏感原件只回传 SHA-256 与必要摘录。
```
...
动态状态和原始证据：对应 PR / Issue / Actions / artifact。
...
- 本机指令和证据默认通过 GitHub Issue/PR 交接，用户不承担常规文件中转；
