...
只保存结构化元数据、摘要和原站附件链接。Mac 批次在分类前用既有受限解析器读取详情正文，限长传入现有分类器并标注截断；提取失败或不足时按标题证据，并用已有来源警告标明原因。正文和 HTML 只留在内存；PENDING 只在分类验证通过后写入白名单决策与列表元数据，不保存原始 AI 响应。附件仍只保存原站链接。附件字节不写入文件、数据库、对象存储或缓存。
...
Issue98采用北京时间05:00、12:00、16:00；沿用`com.capx.newsnow.mac-worker`，入口改为`tools/ai-bridge/collection-tick.mjs`，`StartInterval=60`。每分钟只检查控制队列，不访问信息源或调用AI；指定小时内首次可运行检查执行一次，整小时离线不追补。Mac必须在线且唤醒。调度时段在运行前持久记账，失败可从管理员页面手动重试。

管理员信息源页可排队“立即采集并上线”；活跃任务只允许一个，重复点击不创建第二任务。Mac原子领取后沿用完整worker链路；结果报告丢失时从本地receipt补报，不重复采集。中断后的不确定结果标记失败，人工检查后重试；不自动超时重新领取。

首次切换先确认生产副本干净main、没有活worker/采集锁和进程，精确备份既有plist；保持同一Label、运行目录、日志及环境，只更改入口和轮询周期。卸载旧任务后再次确认空闲，应用通过`plutil -lint`的配置并重新加载；不强杀活任务，不使用第二调度器。云端部署成功与Mac采用新版分别验收。检查一次自然控制轮询无意外采集，再从真实管理员页面排队并读回执行结果。

失效或未知锁保留并停止执行，禁止自动删除；维护者确认任务和子进程均已退出后再处理。receipt与时段记录均执行文件及父目录fsync；重启恢复只补报手动任务结果，不重新执行不确定任务。
