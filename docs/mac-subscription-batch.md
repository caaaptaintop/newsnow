...
本机 Codex 在本文件覆盖的范围内只作为 Mac 执行器：用于读取本机运行副本、`.data`、launchd/plist、PID、锁、日志、CLI 登录态和必须依赖本机浏览器的现场状态。能由 ChatGPT、GitHub、CI、线上接口或云端工具完成的分析、代码修改、PR、复审、合并和部署，不应转交本机 Codex。

Codex 发现代码问题时，默认先复现并保留最小证据，然后停止并交回 ChatGPT；除非 ChatGPT 的明确本机指令授权，否则不自行扩大为代码重构、PR、合并或部署。Codex 自报“通过”不是最终验收，结果仍需由 ChatGPT 独立复审。
