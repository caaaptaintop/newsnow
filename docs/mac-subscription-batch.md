...
常规开发由用户在 Codex 中选定的当前模型负责，原生和网页模型均可完整开发；模型根据需求自动定位或创建分支/worktree，在身份核验后执行文件、Git、Shell、测试与本机工具。网页模型按实际需要核验桥接，原生开发不强制经过 Web。本文件只约束 Mac 生产运行权限；上述自动编排不构成对运行副本、`.data`、launchd/plist、PID、锁、日志、登录态、私钥或生产网络动作的额外授权。

涉及生产运行副本、调度、真实发布或其他本文件运维动作时，继续遵守下文授权、停止条件和验收边界，不自行扩大为生产写入、调度修改、合并或部署。开发模型自审和工具成功退出都不能替代最终独立验收。
