...
## ChatGPT 与本机 Codex 的职责边界

本项目的长期协作规则以仓库根目录 `AGENTS.md` 为准，完整流程说明见 `docs/AI-COLLABORATION.md`。

常规开发以明确的本地 Codex 工作区为默认现场，通过 `codex-chatgpt-web` / MCP 让 ChatGPT Web 模型进入同一 worktree 主导需求、架构、修改、测试设计和 diff 审查，Codex 工具层执行文件、Git、Shell、测试、构建、GUI/macOS 和本机事实动作。本文件只约束 Mac 生产运行相关权限；开发工具层职责不构成对运行副本、`.data`、launchd/plist、PID、锁、日志、CLI 登录态、私钥或生产网络动作的额外授权。

原生 Codex 模型仅按需承担边界清晰的 Mac 专项，不为形式上的独立重复整套开发与审查推理。涉及生产运行副本、调度、私钥、真实发布或其他本文件运维动作时，仍必须遵守下文现有授权、停止条件和验收边界，不得因常规开发职责扩大而自行扩大为生产写入、调度修改、合并或部署。Codex 工具结果和原生 Codex 自报“通过”都不是最终独立验收。
