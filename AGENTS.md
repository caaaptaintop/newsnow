...
`git worktree add` 不改变调用者目录。创建与核验分开调用；核验时把工具 `workdir` 和 `NEWSNOW_EXPECTED_ROOT` 显式绑定到预先确定的同一目标。

若工具记录确认本会话入口已通过身份门，失败仅因仍在入口执行，输出为 canonical 身份及该入口 root，原因严格为 `repository root mismatch; fail-closed`，可以纠正一次 `workdir`。纠正前只读核对失败前已绑定的原目标：路径存在且未漂移、未被替换，仍登记在原共同仓库中，分支和 HEAD 与原记录一致，且无未知修改或写者；任一事实不明即停止。不得临时换目标、回填预期路径或修改 remote。

在当前对话记录这次纠正，以原目标为显式 `workdir`，保留原预期根目录和已核验脚本，重跑完整身份门；通过后继续原授权任务，再次失败立即停止。同一失败最多纠正一次，跨轮不重置，既往次数不明则停止；无需另建次数台账。本节不授予生产写入权限，也不豁免其他批准门。

本条款仅在 NewsNow 试行；用户确认试行结果且完成适用审查前不定版或推广。
...
`用户需求/反馈 → 项目入口核验 → 自动定位或新建 Issue / branch / worktree → 当前选定模型本地开发与验证 → clean fixed candidate → Draft PR → 按第 2.6 节自审或独立审查 → 修复 → final Head → 获得相应授权后 merge main → 部署/生产验收`

`main` 不作为日常开发分支，不直接承载未经审查的开发改动。local-first 只改变开发与验证的执行位置和 GitHub 同步频率，保留 Issue、独立分支、Draft PR 和 main 合并门槛；审查深度按第 2.6 节实际风险选择。
...
3. 按第 2.4 节复用或创建目标，将已核实的绝对路径记为执行根目录。目标不同于入口时显式绑定目标 `workdir` 再次执行身份检查；只有满足第 1.1 节全部条件的工作目录遗漏允许一次限定复核，同一路径且身份事实未变可复用本会话结果。读取目标 `AGENTS.md`、`HANDOFF.md`、`PROGRESS.md`，核对 status、与远端差异和写入归属。同一会话内目标、路径、身份与写入归属未变时复用已核验结果，不因常规修改、测试、反馈或模型切换重复 bootstrap；相同正文只完整读一次，变化处看 diff。新会话、目标切换、交接或疑似并发变化时重新核对相应现场，不依赖聊天记忆。提交/合并前仍核对实际差异与远端状态。
...
- 完成范围匹配的验证和 diff 自审后固定候选；在正式审查、跨设备继续或远端备份等同步点提交/push、创建或更新同一 Draft PR。禁止常规 force push；审查按第 2.6 节分级，轻量修订复用当前任务与同步点，不单独拆出审查任务。
...
按实际行为、影响范围和验证覆盖分类，不因文件名、主题词、改动行数或出现新 Head 自动升级。混合改动按其中实际最高风险处理；执行者在现有 PR 简要说明分类依据和适用验证，无需另建审查申请或风险台账。

**轻量修订**：文字、引用、去重、状态更新，以及不改变执行权限、安全校验、失败停止条件和批准门的调用说明。当前模型完成差异自审、内容一致性/引用检查及适用 lint 后交付。身份调用说明变化涉及未覆盖用法时补相关正反例；已有验证覆盖且脚本、合同未变时可引用固定版本证据。不强制独立审查、新对话、模型切换、全库测试或无关产品 CI。

**普通产品改动**：局部、可逆且不触及下列高风险行为的改动，默认由当前模型完成根因分析、适用回归和自审。存在关键行为无法验证、复杂跨模块影响、已知兼容性缺口，或用户明确要求时，进行独立审查；不能用笼统“已有测试”掩盖未覆盖的风险。普通正文/附件选择器、模板识别修复，在不改变请求安全、持久化和权限边界时按本档评估。已知多模板问题应覆盖相应模板或保留待补证项。

需要普通独立审查时，可使用可用的原生或网页模型，遵从用户选择与宿主编排权限，不强制 Web 提供方。首次审查上下文不得继承开发历史，审查者不得参与被审实现；从仓库与 PR 自行读取固定 SHA 的完整 diff、关键实现、相关测试和适用 CI。桥接不可用时可使用符合独立性条件的可用通道；无法取得必要材料时只阻断依赖该事实的结论和后续动作，不把工具故障本身升级为高级审查。

**高级独立审查**仅由实际高风险行为触发：

- 生产 D1 schema、迁移、数据修复、删除、重建，以及保护这些写入的版本守卫；
- 鉴权、签名 capability、密钥信任、Cloudflare Access 或权限模型变化；
- 生产发布门禁、事务、幂等/CAS、恢复策略变化，可能造成越权发布、数据丢失、重复写入或恢复失败；
- Mac worker 的真实调度、互斥、ledger/receipt/outbox、运行副本切换语义变化，影响生产写入或恢复；
- 放宽正文/HTML/附件的持久化边界、请求信任边界或不可信输入的安全隔离；
- 真实生产写入、不可逆删除、生产恢复操作；
- CI/部署修改绕过或降低实际安全验收门槛；
- 治理改动扩大执行权限、降低安全校验、放宽失败停止条件或降低审查、合并、生产批准门槛。

只涉及这些对象的文字或局部行为时，按实际影响判断，不能仅凭对象名称升级。本地与 GitHub 不一致时先查明是否为未同步、历史快照或版本漂移；无法解释时暂停依赖该证据的动作。只有查明涉及上述高风险行为才升级。

高级审查由用户在 Codex 独立任务中选择模型并启动。执行者一次准备项目入口、固定 Head、范围、风险、验证、已知缺口及 Issue/PR 材料入口，并给出可直接使用的开场说明。执行者不得自行启动、委派或重试高级审查。在同一开发任务切换模型或压缩上下文不构成独立审查。用户尚未启动时只暂停依赖审查的合并，其他已授权工作继续。

**整改复审**：首次独立审查后，优先继续原独立审查任务，核对旧问题、上次已审 SHA 到新 Head 的完整增量及直接回归影响；未变部分可引用明确版本和适用范围的原始证据。结论必须覆盖实际新 Head，但新 SHA 本身不要求全量重审、新模型或新对话。只有原审查者参与实现、原任务不可恢复、风险范围显著扩大或用户要求第二意见时，才另开独立审查。高级复审继续由用户调用原独立任务，执行者提供增量入口，不反复要求重新选择模型或重建上下文。

批准沿用同一对象、动作、内容、影响和费用范围内已有用户授权；用户明确设置的逐次批准门继续有效。审查通过不替代合并或生产授权；本节不授予自动合并或生产操作权限。

改变审查或批准边界的候选仍按变更前门槛验收；用户明确豁免某一次审查时，记录该授权及范围即可，不再请求同一豁免，也不扩展为未来高风险免审。新治理规则先冻结五个真实场景，仅在 NewsNow 试行；用户确认试行结果后才定版或推广。普通文字或状态修订不重复试行手续。
...
- 需求歧义会改变目标；
- 第 2.6 节触发高级独立审查时，通知用户在 Codex 独立对话选择并调用审查模型；
- 真实生产授权、不可逆高价值操作；
...
第 2.6 节要求独立审查的改动，不得以开发自审、工具成功退出或 CI 成功替代。

## 4. 缺陷修复、最低测试矩阵与代码规则

缺陷默认闭环：

`复现 → 根因 → 失败回归 → 最小修复 → 回归 → diff 自审 → fixed candidate → 按第 2.6 节完成审查`
...
仅修改文档且身份脚本、调用合同与工作流未变时，不重跑产品测试、构建或历史全库诊断。开发隔离测试可即时执行；生产自然周期要求不延伸到隔离测试，生产调度保护保持不变。

已有全库范围外诊断可以单列；**“改动文件 0 新错误”不得表述为“全库无错误”**。历史 CI 成功只对应其固定 SHA，不替代后续 Head 的验证。

最终合并前至少核对：实际 Head SHA、base、changed files、完整关键 diff、最低测试矩阵中适用项、适用 CI 与审查结果、已知边界和并行提交。按第 2.6 节无需独立审查的候选记录自审与分类依据即可；已有必需检查不得跳过。不能只信 PR 描述、绿色标记或 Codex 总结。
...
- `HANDOFF.md`：本阶段接续增量，最多 12 行，包含目标/Issue、阶段、已完成、未完成、验证入口、风险、归属和下一动作；仅在对复现必要时记录固定基线，机器路径及活动映射留本机 `.local/CONTEXT.md`；
- `docs/AI-COLLABORATION.md`：用户流程、例子与冻结验收场景；规范清单引用本文件；
- `docs/mac-subscription-batch.md`：Mac 生产运维细则。

PR 是否已创建、远端 Head、CI、合并和关闭状态在接续时直接查 Git/GitHub。状态文件只在目标、决策、归属、剩余工作或证据入口实质变化时更新；提交前写的快照注明适用阶段，不要求正文包含自身 commit SHA，也不为追赶 push/PR/CI 状态制造新提交。过期文字会误导执行时做轻量更正，不把可查证的快照滞后列为产品阻断缺陷；目标版本无法确认、证据错配或真实权限冲突仍须停止对应动作并查明。
