...
按行为和权限影响分类，不因修改 `AGENTS.md` 或包含“流程”“恢复”等词自动升级；混合改动按最高风险处理。

**轻量修订**：仅文字、引用、去重、状态更新，或执行权限、安全校验、失败停止条件与批准门均不变的调用说明。当前模型完成差异自审、内容一致性/引用检查及适用 lint 后即可交付；涉及身份调用说明时补跑相关正反例。不强制独立审查、新对话、模型切换、全库测试或无关产品 CI 取证；保留 Git/PR 留痕和已有合并授权边界。执行权限、停止条件或审查门槛发生变化时不适用此档。

**普通产品改动**：保持以下独立审查流程。涉及高风险清单的行为变化时升级；清单中的对象仅被文字提及或说明整理，且不改变行为与权限时，仍可按轻量修订处理。
...
- 治理改动扩大执行权限、降低安全校验、放宽失败停止条件，或取消/降低原有审查、合并及生产批准门槛。

高风险任务由当前选定模型在授权范围内开发和验证。需要高级独立审查时，执行模型先固定待审 Head，将范围、风险、测试、CI、已知问题和材料入口写入现有 Issue/PR，并通知用户；由用户在 Codex 独立对话中选择并调用审查模型；遵从用户选择本地或网页模型。执行模型不得自行启动、委派或重试高级审查，也不得以普通审查代替该步骤。用户调用后的审查模型从项目与 GitHub 自行读取材料、核对目标工作树和固定 SHA；采用未继承开发历史的新上下文，不要求用户搬运开发内容。在同一开发对话直接切换模型不构成独立审查。
...
改变审查或批准边界的规则候选，按变更前门槛验收，不得引用自身新增例外免审。新治理规则先冻结五个实际场景，只在 NewsNow 试行；用户确认试行结果后才能定版或推广。普通文字修订不重复这一试行流程。
...
最终合并前至少核对：实际 Head SHA、base、changed files、完整关键 diff、最低测试矩阵中适用项、适用 CI 与审查结果、已知边界和并行提交。轻量修订记录自审结果即可；已有必需检查不得跳过。不能只信 PR 描述、绿色标记或 Codex 总结。
...
- 附件字节不得长期写入 Git、D1、对象存储或证据包。用户授权的按需预览可使用短期边缘缓存：仅成功且不超过 5 MiB 的附件，逻辑有效期最多 5 分钟；每次访问仍核对已收录文章和预算，过期不可读取，失败页不缓存。浏览器响应保持 no-store；不得自动启用付费存储或超预算继续处理；
- PDF 附件直接在新标签打开官方文件，不走本站解析；其他附件先预览再决定下载，优先浏览器直连，必要时使用受限临时缓存与 relay；
