...
function normalizedUrl(value: unknown, field: string) {
  ...
  if (host === "localhost" || host.endsWith(".localhost") || /^\d{1,3}(?:\.\d{1,3}){3}$/.test(host) || host.includes(":")) {
    throw new Error(`${field} 不允许本机或 IP 地址`)
  }
  url.hash = ""
  return url.href
}
...
export function validateIntelligenceSourceConfig(value: unknown, seed: IntelligenceSource): IntelligenceSourceConfig {
  ...
  if (rawEndpoints.length > 6) throw new Error("单个来源最多配置 6 个栏目")
  ...
  if (mode === "feed" && !seed.newsnowId) throw new Error("该来源不支持结构化信息流模式")
  return {
    schemaVersion: 1,
    id: seed.id,
    topic: seed.topic,
